...
class MCPClient:
    ...
    async def call_tool(
        ...
        # Call a particular tool from the MCP server and return the result
    ...
    async def read_resource(self, uri: str) -> Any:
        ...
        resource = result.contents[0] # gets the 1st dict (JSON) returned.

        if isinstance(resource, types.TextResourceContents):
            if resource.mimeType == "application/json":
                return json.loads(resource.text) # parse the text as JSON
        
            return resource.text # returned as plain text, no parsing
...
# For testing: mcp uv run mcp_client.py
async def main():
    async with MCPClient(
        # If using Python without UV, update command to 'python' and remove "run" from args.
        command="uv", args=["run", "mcp_server.py"],
    ) as _client:
        # starts a copy of MCP server, gets list of all tools, print result
